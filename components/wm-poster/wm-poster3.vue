...
export default {
	...
	methods: {
		async OnCanvas() {
			...
			// let _QrCode =  _this.QrSrc //二维码或太阳码
			...
			let imgL = uni.upx2px(270)
			let imgT = uni.upx2px(730)
			_this.ctx.drawImage(_this.qrBase64, imgL, imgT, q[0], q[1]);
			//添加二维码 end
			...
			_this.ctx.setFillStyle(_this.OriginalColor);
			_this.ctx.setFontSize(uni.upx2px(24));
			let TextL = uni.upx2px(270)
			_this.ctx.fillText(_this.ViewDetails, TextL, _strHeight + q[1] / 2);
			//添加推荐人与描述 end
