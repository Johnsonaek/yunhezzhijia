<template>
	<view>
		<page>
			<view class="">
				<!-- 轮播图 -->
				<swiper style="height: 750rpx;" class="round-dot" :indicator-dots="true"  :circular="true" :autoplay="true" interval="5000" duration="500">
					<swiper-item v-for="(item, index) in detail.goods_gallery_urls" :key="index">
						<navigator hover-class="none" :url="item.url" class="radius12">
							<q-image :lazyLoad="false" :imgwh="[750, 750]" :isRatio="true" :src="item"></q-image>
						</navigator>
					</swiper-item>
				</swiper>
			</view>
			...
			<view class="bg-white plr30 ptb30">
				...
				<view class="flex a-center mt30">
					<view class="flex-item">
						...
						<text class="text-red fs40">{{detail.min_group_price - detail.coupon_discount }}</text>
					</view>
				...
				<view class="flex a-center mt40">
					<view class="w200 text-muted fs24">原价：<text class="text-muted line-through fs24">￥{{detail.min_group_price}}</text></view>
				...
				<view @click="openUrl(detail.couponurl)" class="s-coupon flex a-center" style="background-image: url(/static/svideo/coupon.png);">
					<view class="w160 aj-center flex">
						<view class="fs30 text-yellow">
							<text class="numcolor">￥</text>
							<text class="num numcolor" style="font-size: 50rpx;">{{detail.coupon_discount}}</text>
						</view>
					</view>
					<view class="h120 bor-1px-r mr30"></view>
					<view class="flex-item">
						<view class="fs30 text-white">满{{detail.couponexplain}}使用</view>
						<view class="fs24 text-fff50 mt20">使用时间：{{detail.coupon_start_time | t}}-{{detail.coupon_end_time | t}}</view>
					</view>
				</view>
			...
			<view class="qui-cells mt30">
				<view @click="openUrl(mallUrl.mobile_short_url)" class="qui-cell border-none">
					<view class="mr30 text-gray">
						<image class="wh70 block radius50" src="/static/images/default-tx.png" mode=""></image>
					</view>
					<view class="cell-bd fs30">{{detail.mall_name}}</view>
			...
			<view slot="bottom" class="flex bg-white bor-1px-t shadow-xy0">
				...
				<view class="flex-item flex">
					...
					<view @click="openUrl(mallUrl.mobile_url)" class="text-white flex-item h100 flex aj-center" style="background-color: #F82840;">
						<view class="tc">
							￥{{detail.min_group_price - detail.coupon_discount }}
							<view>自购省</view>
						</view>
					</view>
				</view>
...
export default {
	...
	methods: {
		...
		async getDetail(){
			const res = await this.post('/wap/Coalition/PddDetails',{goods_id_list: this.id})
			console.log(res)
			if(res.code == 200) {
		...
		async getMallUrl() {
			const res = await this.post('/wap/Coalition/PddMallUrl',{mall_id: this.id})
			this.mallUrl = res.data.list[0]
		}
