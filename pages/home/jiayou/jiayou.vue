	...
	<view>
		<el-dialog
		  ...
		  <view class="dialoginfo">
			  ...
			  <view class="dialogname">
				  品牌优选-{{dialogObj.name}}
			  </view>
		  </view>
		  <el-button type="primary" color="#E41937" @click="dialogTableVisible = false">知道了</el-button>
		</el-dialog>
		<view class="content">
			<page title="一键加油">
				...
				<view class="jycontent" v-if="active == 0">
					...
					<ul class="infinite-list" v-infinite-scroll="load" style="overflow:auto" infinite-scroll-disabled="disabled" v-if="!qiyouListShow && !fanshitrue">
					   <li v-for="(item,index) in loadgasStation" class="infinite-list-item" :key="item.id" @click="gotoGasStation(item.range,item.gasId)"><view class="jiayouitem">
					   
							<view class="gasStation_positioning_top">
								<view class="gasStation_name">
									{{item.gasName}}
								</view>	
							...
							<view class="gasStation_positioning_bottom">
								...
								<view class="falling_icon">
									以降￥{{'0'+(item.priceGun-item.priceYfq).toFixed(2)}}
								</view>
								<del class="del_price">
									国际价￥{{item.priceOfficial}}
								</del>
							</view>
						</view></li>
					</ul>
					...
					<view class="oilChooseList" v-if="qiyouListShow">
						<view class="qiyou">
							...
							<view class="qiyoulist">
								<view :class="{chaiyou_item:true,item:true,choose:item+'#' == oliRadio || item == oliRadio}" v-for="(item,index) in qiyouList" :key="index" @click="olichoose(item,index)">{{item}}#</view>
							</view>
						</view>
						<view class="chaiyou">
							<view class="title">柴油</view>
							<view class="chaiyoulist">
								<view :class="{chaiyou_item:true,item:true,choose:item+'#' == oliRadio || item == oliRadio}" v-for="(item,index) in chaiyouList" :key="index" @click="olichoose(item,index)">{{item}}#</view>
							</view>
						</view>
						<view class="trqi">
							<view class="title">天然气</view>
							<view class="trqilist">
								<view :class="{chaiyou_item:true,item:true,choose:item+'#' == oliRadio || item == oliRadio}" v-for="(item,index) in trqiList" :key="index" @click="olichoose(item,index)">{{item}}</view>
							</view>
						</view>
					</view>
					
					<view class="fanshiList" v-if="fanshitrue">
						<view :class="{fanshi_item:true,choose:index == fanshiRadio} " v-for="(item,index) in fanshiList" :key="index" @click="fanshichoose(index)">{{item.name}}</view>
					</view>
